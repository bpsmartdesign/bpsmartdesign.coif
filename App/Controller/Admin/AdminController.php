<?php
	...
	namespace ExpHairActivityManager\Controller\Admin;


	class AdminController extends \ExpHairActivityManager\Controller\AppController {
		...
		private function adminVerification($type = 'personnel') {
			...
			if(!isset($_SESSION['aedjkexphairaKDAIkjeILAODkljeiLEKA4ç_H'])) {
		...
		public function controlLog() {
			...
			if(isset($_SESSION['id_lkjaldkafpoijdmfaexphair'])) {
				...
				if(sha1(md5($this->secureData($_POST['password']))) == $actualUser->password) {
					$_SESSION['aedjkexphairaKDAIkjeILAODkljeiLEKA4ç_H'] = $username;
